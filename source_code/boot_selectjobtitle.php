...
//For our job drop down menu
$sqlselectj = "SELECT * from job ORDER by dbjobName";
...
if( isset($_POST['thesubmit']) )
	...
	{
		//select clause for job titles
		$sqlselect = 'SELECT job.*, permission.* FROM job, permission 
			WHERE job.dbpermissionID = permission.dbpermissionID
			AND job.dbjobID = :bvjobid';
		$result = $db->prepare($sqlselect);
		$result->bindValue(':bvjobid', $formfield['ffjobname']);
		$result->execute();
                        ...
                        <form name = "theform" id="theform" method= "post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="row g-4">
                                            <div class="col-12 pb-2">
											 <div class="form-group">
												<label for="jobname" class ="text-light pb-2" style="font-family: 'Raleway', sans-serif;">Food Category</label>
												<select class="form-select" id="jobname" name="jobname">
												<option value="">SELECT JOB TITLE</option>
												<?php while ($rowj = $resultj->fetch() )
												{
												if ($rowj['dbjobID'] == $formfield['ffjobname'])
												{$checker = 'selected';}
												else {$checker = '';}
												echo '<option value="'. $rowj['dbjobID'] . '"' . $checker . '>' . 
												$rowj['dbjobName'] . '</option>';
												}
												?>
												</select>
											 </div>
                           ...
                           <div class="col-md-5 ps-0 d-none d-md-block">
                                <div class="form-right border border-dark border-2 rounded-3 h-100 bg-white text-dark text-center pt-2">
                                    			<img id="joblogo" alt="Job Logo" class = "pt-2" style="height: 240px; vertical-align: middle;" src="./images/joblogo3.svg">
	...
	while ($row = $result-> fetch() )
		{
				...
				. '</td><td>' . $row['dbpermissionDescription'] . '</td><td>' .
				//This will make a button that sends the user to another page "updatejobtitle.php"
				'<form action="boot_updatejobtitle.php" method="post">
...
{	
?>
<!--default formtype when no records are pulled---->
<div class="d-flex flex-column min-vh-100 justify-content-left align-items-left">
        <div class="container">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
...
{	
?>
<!--default formtype when records are pulled---->
<div class="d-flex flex-column min-vh-100 justify-content-left align-items-left">
        <div class="container">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
