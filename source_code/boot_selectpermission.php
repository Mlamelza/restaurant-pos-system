<?php
$pagetitle = 'Select Permission Role';
...
$sqlselectp = "SELECT * from permission ORDER by dbpermissionTitle";
...
essentially start over and select a permission record from the DB.
**/
...
if( isset($_POST['thesubmit']) )
	{
		//Data Cleansing
		$formfield['ffpermitname'] = trim($_POST['permitname']);
		/*****************************************************************************
     	CHECK FOR EMPTY FIELDS
    	Complete the lines below for any REQIURED form fields only.
		Do not do for optional fields.
		****************************************************************************/
		if(empty($formfield['ffpermitname']))
		{
			$errormsg .= 'Your Permission Title is empty! ';
			}	
	...
	{
		//select clause for permission titles
		$sqlselect = 'SELECT * FROM permission WHERE dbpermissionID
			= :bvpermitid';
		$result = $db->prepare($sqlselect);
		$result->bindValue(':bvpermitid', $formfield['ffpermitname']);
		$result->execute();
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
                  <h3 class="mb-3 pt-3" style = "font-family: 'Ysabeau SC', sans-serif;">Permission Role Information</h3>
                        ...
                        <form name = "theform" id="theform" method= "post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="row g-4">
                                            <div class="col-12 pb-2">
											 <div class="form-group">
												<label for="permitname" class ="text-light pb-2" style="font-family: 'Raleway', sans-serif;">Permission Role Title</label>
												<select class="form-select" id="permitname" name="permitname">
												<option value="">SELECT PERMISSION</option>
												<?php while ($rowp = $resultp->fetch() )
												{
												if ($rowp['dbpermissionID'] == $formfield['ffpermitname'])
												{$checker = 'selected';}
												else {$checker = '';}
                           ...
                           <div class="col-md-5 ps-0 d-none d-md-block">
                                <div class="form-right border border-dark border-2 rounded-3 h-100 bg-white text-dark text-center pt-2">
                                    			<img id="permitlogo" alt="Permit Logo" class = "pt-2" style="height: 240px; vertical-align: middle;" src="./images/placemat2.svg">
...
/***************************************************************************************/
/*Here, the form that will display indicates the fact that the submit button
was pressed by the user and there was at least one record found.*/
...
while ($row = $result-> fetch())
	{		
	echo '<tr><td>' . $row['dbpermissionTitle'] . '</td><td>' . $row['dbpermissionDescription'] 
	. '</td><td>' .
	//This will make a button that sends the user to another page "updatepermission.php"
	'<form action="boot_updatepermission.php" method="post">
		<input type="hidden" name="permitid" value="'. $row['dbpermissionID'] .'">
...
/*The following default formtype 1 is displayed for Regional Managers*/
if (($showform == 7) && ($formtype == 1))
{	
?>
<!--default formtype 1---->
<div class="d-flex flex-column min-vh-100 justify-content-left align-items-left">
...
/***************************************************************************************/
/*Here, the form that will display indicates the fact that the submit button
was pressed by the user (Regional) and there was at least one record found.*/
if (($showform == 7) && ($formtype == 3))
{	
?>
<div class="d-flex flex-column min-vh-100 justify-content-left align-items-left">
        <div class="container">
...
//Remaining logged in users
if (($showform == 1) || ($showform == 2) || ($showform == 3) ||
		($showform == 6) || ($showform == 8)) {
	echo '<div class = "container ps-5">';
	echo '<div class="message alert alert-info fs-5 border-dark" role="alert">';
