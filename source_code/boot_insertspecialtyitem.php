<?php
$pagetitle = "Add Specialty Item: Specific Location(s)";
...
$sqlselectc = "SELECT * from category WHERE dbcategoryID 
...
item when added. The menu item will have a unique id, but will be associated
with multiple locations and price points, depending on the location.*/
...
if (isset($_POST['thenewsubmit']))	{
	...
	if( isset($_POST['thesubmit']) )
	{
		...
		//Data Cleansing
	//	$formfield['ffitemtype'] = trim($_POST['itemtype']);
		...
		$itemlocations = $_POST['itemlocations'];//location array
    	...
    	**************************************************************************** */
	//	if(empty($formfield['ffitemtype'])){$errormsg .= "<p>Your item type field is empty.</p>";}	
	...
	//	if(empty($formfield['ffitemdescr'])){$errormsg .= "<p>Your item description field is empty.</p>";}
		...
		if(empty($formfield['ffitembutton'])){$errormsg .= "Your Menu Item Button field is empty! ";}
	...
	if(($itemlocations == NULL) || (in_array(0, $itemlocations))){
		$errormsg .= "Your Location choice is empty! ";
	}
        ...
        $errormsg .= "Menu item already exists. Please enter a new menu item! ";
			...
			**************************************************************************** */
			...
			else
			{
				...
				$stmtinsert->bindvalue(':bvitemtype', 2);
			//	$stmtinsert->bindvalue(':bvitemtype', $formfield['ffitemtype']);
				$stmtinsert->execute();
                  ...
                  <h3 class="mb-3 pt-3" style = "font-family: 'Ysabeau SC', sans-serif;">Specialty Menu Item Information</h3>
                    <div class="bg-dark shadow rounded-3">
                        ...
                        <form name = "theform" id="theform" method= "post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="row g-4">
											...
											<div class="col-12 pt-2">
                                                <label for = "itembutton" class ="text-light" style="font-family: 'Raleway', sans-serif;">Menu Item Button Name<span class="text-light">*</span></label>
                                                <div class="input-group">
                                                    <div class="input-group-text"></div>
                                                    <input type="text" name ="itembutton" id="itembutton" class="form-control" placeholder = "Enter Item Button Name" required >
                                                </div>
                                            </div>
                                ...
                                <div class="right h-100 py-5 px-5 border border-dark border-2 rounded-3 h-100 bg-white text-dark text-left pt-5">
										<div class="col-12 pb-3">
											 <div class="form-group border-dark">
												<label for="itemlocations" class ="text-dark" style="font-family: 'Raleway', sans-serif;">Select Location(s)<span class="text-dark">*</span></label>
												<select class="form-select border-dark" id="itemlocations[]" name="itemlocations[]" multiple size = 5 required>
										<!--		<option value="">SELECT LOCATION(S)</option>-->
												<?php //Notice here that our query is pulling total locations
					//that the current logged in user is associated with, in this
					//case, all locations are essentially pulled in our checklist
					//because this form display indicates Admin privileges.
					while ($rowselectlo = $resultselectloc->fetch() )
						{
			//				if ($rowselectlo['dblocationID'] == $itemlocations)
							if (in_array(($rowselectlo['dblocationID']), $itemlocations))
							{$checker = 'selected';}
							else {$checker = '';}
							echo '<option value="'. $rowselectlo['dblocationID'] . '"' . $checker . '>' . 
							$rowselectlo['dblocationName'] . '</option>';
						}
						?>
												</select>
											 </div>
...
{
?>
<!---form to display when there is an error message----->
<div class="d-flex flex-column min-vh-100 justify-content-left align-items-left">
        <div class="container">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
                    ...
                    <div class="bg-dark shadow rounded-3">
                        ...
                        <form name = "theform" id="theform" method= "post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="row g-4">
											...
											<div class="col-12 pt-2">
                                                ...
                                                <div class="input-group">
                                                    <div class="input-group-text"></div>
                                                    <input type="text" name ="itemname" id="itemname" class="form-control" value="<?php echo $formfield['ffitemname']; ?>">
                                                </div>
                                            </div>
											<div class="col-12 pt-2">
                                                <label for = "itembutton" class ="text-light" style="font-family: 'Raleway', sans-serif;">Menu Item Button Name<span class="text-light">*</span></label>
                                                <div class="input-group">
                                                    <div class="input-group-text"></div>
                                                    <input type="text" name ="itembutton" id="itembutton" class="form-control" value="<?php echo $formfield['ffitembutton']; ?>"  >
                                                </div>
                                            </div>
                            ...
                            <div class="col-md-5 ps-0 d-none d-md-block">
                                <div class="right h-100 py-5 px-5 border border-dark border-2 rounded-3 h-100 bg-white text-dark text-left pt-5">
										<div class="col-12 pb-3">
											 <div class="form-group border-dark">
												<label for="itemlocations" class ="text-dark" style="font-family: 'Raleway', sans-serif;">Select Location(s)<span class="text-dark">*</span></label>
												<select class="form-select border-dark" id="itemlocations[]" name="itemlocations[]" multiple size = 5>
										<!--		<option value="">SELECT LOCATION(S)</option>-->
												<?php //Notice here that our query is pulling total locations
					//that the current logged in user is associated with, in this
					//case, all locations are essentially pulled in our checklist
...
?>
<!---form displayed when successful database insert completed---->
<div class="d-flex flex-column min-vh-100 justify-content-left align-items-left">
        <div class="container">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
                        ...
                        <form name = "theform" id="theform" method= "post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="row g-4">
											...
											<div class="col-12 pt-2">
											 <div class="form-group">
												...
												<?php while ($rowc = $resultc->fetch() )
												{
												if ($rowc['dbcategoryID'] == $row['dbcategoryID'])
												{$checker = 'selected';}
												else {$checker = '';}
												echo '<option value="'. $rowc['dbcategoryID'] . '"' . $checker . '>' . 
												$rowc['dbcategoryName'] . '</option>';
											...
											<div class="col-12 pt-2">
                                                ...
                                                <div class="input-group">
                                                    <div class="input-group-text"></div>
                                                    <input type="text" name ="itemname" id="itemname" class="form-control" value="<?php echo $row['dbmenuitemName']; ?>">
                                                </div>
                                            </div>
											<div class="col-12 pt-2">
                                                <label for = "itembutton" class ="text-light" style="font-family: 'Raleway', sans-serif;">Menu Item Button Name<span class="text-light">*</span></label>
                                                <div class="input-group">
                                                    <div class="input-group-text"></div>
                                                    <input type="text" name ="itembutton" id="itembutton" class="form-control" value="<?php echo $row['dbmenuitemButton'];?>" >
                                                </div>
                                            </div>
...
<?php
	...
	echo '<tr><td>' . $row['dbmenuitemTypeName'] . '</td><td>' . $row['dbcategoryName'] . '</td><td>' . $row['dbmenuitemName'] . 
		...
		'</td><td>' . $row['dbinventoryDate'] . '</td><td>$' . $row['dbmenuitemDetailCost'] . '</td><td>' . $row['dbfoodstatusName'] . '</td></tr></tbody>';
		
		echo '<thead class="thead-dark border border-dark bg-dark text-light">';
		echo '<tr>';
		while ($locationcount > 0){
		echo'<th scope="col" style = "font-family: "Raleway", sans-serif;">Location</th>';
		
		$locationcount = $locationcount - 1;}
		
		echo '</tr></thead>';
	
		echo '<tbody class = "border border-dark">';
		...
		echo '<td>' . $row3['dblocationName'] . '</td>';}
		echo '</tr>';
		echo '</thead>';
?>
</tbody>
