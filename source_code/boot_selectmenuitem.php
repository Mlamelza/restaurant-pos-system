...
$sqlselectcat = "SELECT * from category WHERE category.dbcategoryID 
...
$sqlselecttype = "SELECT * from menuitemType";
$resulttype = $db->prepare($sqlselecttype);
$resulttype->execute();

//For our regular menu item text box below
$sqlmenutype = "SELECT * from menuitemType WHERE menuitemType.
	dbmenuitemTypeID = 1";
$resultmenutype = $db->prepare($sqlmenutype);
$resultmenutype->execute();
$rowmenutype = $resultmenutype->fetch();
...
if( isset($_POST['thesubmit']) )
{
	...
	}//ends admin showform 5
	
	//all other permissible users not including admins
	...
	if ((empty($formfield['ffitemname'])) && (empty($formfield['ffitemcat'])) && (empty($formfield['ffitemloc'])) 
		&& (empty($formfield['ffitemstat'])) && (empty($formfield['ffitemtype']))) {
					
				$errormsg .= "You must choose at least one search method! ";
				}
	
	} //ends else for all other permissible users
...
//select clause for all other users not including admins
if (($showform == 1)|| ($showform == 2) || ($showform == 3) ||
		($showform == 6) || ($showform == 7) || ($showform == 8)) {
/*	$sqlselect = "SELECT menuitem.*, foodstatus.*, category.*, menuitemType.*,
		location.* FROM menuitem, foodstatus, category, location, menuitemDetail,
		menuitemType WHERE menuitemDetail.dbmenuitemID = menuitem.dbmenuitemID AND  
		menuitem.dbcategoryID = category.dbcategoryID AND category.dbcategoryID 
		= :bvitemcat AND location.dblocationID = :bvitemloc AND
		...
		foodstatus.dbfoodstatusID = :bvitemstat AND menuitem.dbmenuitemName 
		like CONCAT('%', :bvitemname,'%')";*/
		
	$sqlselect = "SELECT menuitem.*, foodstatus.*, category.*, menuitemType.*, menuitemDetail.*,
		location.* FROM menuitem, foodstatus, category, location, menuitemDetail,
		menuitemType WHERE menuitemDetail.dbmenuitemID = menuitem.dbmenuitemID AND  
		menuitem.dbcategoryID = category.dbcategoryID AND category.dbcategoryID 
		= :bvitemcat AND location.dblocationID = :bvitemloc AND
	...
	$result->bindValue(':bvitemtype', $formfield['ffitemtype']);
	$result->execute();
	}
	//for admin users only, which will simply pull the menu item
	else if ($showform == 5) {
/*	$sqlselect = "SELECT menuitem.*, foodstatus.*, category.*, menuitemType.*,
		location.* FROM menuitem, foodstatus, category, location, menuitemDetail,
		menuitemType WHERE menuitemDetail.dbmenuitemID = menuitem.dbmenuitemID AND  
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-10 offset-lg-1">
                  <h3 class="mb-3" style = "font-family: 'Ysabeau SC', sans-serif;">Menu Item Information</h3>
						...
						<form name = "theform" id="theform" method= "post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="row g-4">
                                            <div class="col-12">
                                                ...
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" name ="itemtype" id="itemtype" class="form-control" value ="<?php echo $rowmenutype['dbmenuitemTypeName']; ?>" 
														 readonly>
                                                </div>
                                            </div>
											<div class="col-12">
...
/***************************************************************************************/
/*Here, the form that will display indicates the fact that the submit button was pressed 
by the user and there was at least one record found, with the results shown below.*/
	...
	while ($row = $result-> fetch() )
			{
				...
				'<form action="boot_updatemenuitem.php" method="post">
...
//For all other permissible users with default formtype 1. Notice the location 
//dropdown is shown in this form, including all locations associated with user.
else if ((($showform == 1) || ($showform == 2) || ($showform == 3) ||
                  ...
                  <h3 class="mb-3" style = "font-family: 'Ysabeau SC', sans-serif;">Menu Item Information</h3>
                    <div class="bg-dark shadow rounded-3">
						...
						<form name = "theform" id="theform" method= "post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="row g-4">
                                            <div class="col-12">
											 <div class="form-group">
												<label for="itemtype" class ="text-light" style="font-family: 'Raleway', sans-serif;">Food Category</label>
												<select class="form-select" id="itemtype" name="itemtype">
											<!--	<option value="">SELECT MENU TYPE</option>-->
												<?php while ($rowtype = $resulttype->fetch() )
												{
												if ($rowtype['dbmenuitemTypeID'] == $formfield['ffitemtype'])
												{$checker = 'selected';}
												else {$checker = '';}
												echo '<option value="'. $rowtype['dbmenuitemTypeID'] . '"' . $checker . '>' . 
												$rowtype['dbmenuitemTypeName'] . '</option>';
												}
												?>
												</select>
											 </div>
...
//All other permissible users with formtype 2, when no results are found.
//This displays when the submit button has been pressed.
else if ((($showform == 1) || ($showform == 2) || ($showform == 3) ||
...
//All other permissible users with formtype 3, when results are found.
//This displays when the submit button has been pressed.
else if ((($showform == 1) || ($showform == 2) || ($showform == 3) ||
